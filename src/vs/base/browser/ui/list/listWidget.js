...
const aria_1 = require("vs/base/browser/ui/aria/aria");
const splice_1 = require("vs/base/browser/ui/list/splice");
...
class KeyboardController {
	...
	multipleSelectionDisposables = new lifecycle_1.DisposableStore();
	get onKeyDown() {
		return this.disposables.add(event_2.Event.chain(this.disposables.add(new event_1.DomEmitter(this.view.domNode, 'keydown')).event)
...
class TypeNavigationController {
	...
	enable() {
		...
		let typing = false;
		const onChar = this.enabledDisposables.add(event_2.Event.chain(this.enabledDisposables.add(new event_1.DomEmitter(this.view.domNode, 'keydown')).event))
			.filter(e => !isInputElement(e.target))
		...
		for (let i = 0; i < this.list.length; i++) {
			...
			if (this.list.options.typeNavigationEnabled) {
				if (typeof labelStr !== 'undefined') {
					const prefix = (0, filters_1.matchesPrefix)(word, labelStr);
...
class DOMFocusController {
	...
	constructor(list, view) {
		this.list = list;
		this.view = view;
		const onKeyDown = this.disposables.add(event_2.Event.chain(this.disposables.add(new event_1.DomEmitter(view.domNode, 'keydown')).event))
			.filter(e => !isInputElement(e.target))
			.map(e => new keyboardEvent_1.StandardKeyboardEvent(e));
...
class MouseController {
	...
	mouseSupport;
	disposables = new lifecycle_1.DisposableStore();
	_onPointer = new event_2.Emitter();
...
class List {
	...
	get onContextMenu() {
		...
		const fromKeyDown = this.disposables.add(event_2.Event.chain(this.disposables.add(new event_1.DomEmitter(this.view.domNode, 'keydown')).event))
			.map(e => new keyboardEvent_1.StandardKeyboardEvent(e))
			...
			.filter(() => false)
			.event;
		const fromKeyUp = this.disposables.add(event_2.Event.chain(this.disposables.add(new event_1.DomEmitter(this.view.domNode, 'keyup')).event))
			...
			.event;
		const fromMouse = this.disposables.add(event_2.Event.chain(this.view.onContextMenu))
	...
	_onDidDispose = new event_2.Emitter();
