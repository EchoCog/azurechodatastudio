...
const findInput_1 = require("vs/base/browser/ui/findinput/findInput");
const inputBox_1 = require("vs/base/browser/ui/inputbox/inputBox");
const listView_1 = require("vs/base/browser/ui/list/listView");
const listWidget_1 = require("vs/base/browser/ui/list/listWidget");
const toggle_1 = require("vs/base/browser/ui/toggle/toggle");
const indexTreeModel_1 = require("vs/base/browser/ui/tree/indexTreeModel");
const tree_1 = require("vs/base/browser/ui/tree/tree");
...
const filters_1 = require("vs/base/common/filters");
...
class TreeNodeListDragAndDrop {
    ...
    dnd;
    autoExpandNode;
    autoExpandDisposable = lifecycle_1.Disposable.None;
...
class TreeRenderer {
    ...
    indentGuidesDisposable = lifecycle_1.Disposable.None;
    disposables = new lifecycle_1.DisposableStore();
...
class FindWidget extends lifecycle_1.Disposable {
    tree;
    elements = (0, dom_1.h)('.monaco-tree-type-filter', [
    ...
    right = 0;
    top = 0;
    _onDidDisable = new event_2.Emitter();
    ...
    constructor(container, tree, contextViewProvider, mode, matchType, options) {
        ...
        const onKeyDown = this._register(event_2.Event.chain(emitter.event))
        ...
        const onGrabKeyDown = this._register(event_2.Event.chain(this._register(new event_1.DomEmitter(this.elements.grab, 'keydown')).event))
            .map(e => new keyboardEvent_1.StandardKeyboardEvent(e))
            .event;
...
class FindController {
    ...
    widget;
    width = 0;
    _onDidChangeMode = new event_2.Emitter();
    onDidChangeMode = this._onDidChangeMode.event;
    _onDidChangeMatchType = new event_2.Emitter();
    onDidChangeMatchType = this._onDidChangeMatchType.event;
    _onDidChangePattern = new event_2.Emitter();
    onDidChangePattern = this._onDidChangePattern.event;
    _onDidChangeOpenState = new event_2.Emitter();
    onDidChangeOpenState = this._onDidChangeOpenState.event;
    enabledDisposables = new lifecycle_1.DisposableStore();
    disposables = new lifecycle_1.DisposableStore();
...
class Trait {
    ...
    nodes = [];
    elements;
    _onDidChange = new event_2.Emitter();
...
class AbstractTree {
    ...
    selection;
    anchor;
    eventBufferer = new event_2.EventBufferer();
    findController;
    onDidChangeFindOpenState = event_2.Event.None;
    ...
    _onWillRefilter = new event_2.Emitter();
    ...
    _onDidUpdateOptions = new event_2.Emitter();
    ...
    constructor(_user, container, delegate, renderers, _options = {}) {
        ...
        onDidChangeActiveNodes.input = event_2.Event.chain(event_2.Event.any(onDidModelSplice, this.focus.onDidChange, this.selection.onDidChange))
            .debounce(() => null, 0)
            .map(() => {
            const set = new Set();
        ...
        if (_options.keyboardSupport !== false) {
            const onKeyDown = event_2.Event.chain(this.view.onKeyDown)
