...
class Storage extends lifecycle_1.Disposable {
	...
	_onDidChangeStorage = this._register(new event_1.PauseableEmitter());
	onDidChangeStorage = this._onDidChangeStorage.event;
	state = StorageState.None;
	cache = new Map();
	flushDelayer = new async_1.ThrottledDelayer(Storage.DEFAULT_FLUSH_DELAY);
...
class InMemoryStorageDatabase {
	onDidChangeItemsExternal = event_1.Event.None;
	items = new Map();
	async getItems() {
		return this.items;
	}
