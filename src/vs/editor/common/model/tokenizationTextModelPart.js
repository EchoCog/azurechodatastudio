...
const languages_1 = require("vs/editor/common/languages");
const textModelPart_1 = require("vs/editor/common/model/textModelPart");
const textModelTokens_1 = require("vs/editor/common/model/textModelTokens");
const contiguousMultilineTokensBuilder_1 = require("vs/editor/common/tokens/contiguousMultilineTokensBuilder");
const contiguousTokensStore_1 = require("vs/editor/common/tokens/contiguousTokensStore");
const sparseTokensStore_1 = require("vs/editor/common/tokens/sparseTokensStore");
class TokenizationTextModelPart extends textModelPart_1.TextModelPart {
	...
	_languageId;
	_attachedViews;
	_semanticTokens = new sparseTokensStore_1.SparseTokensStore(this._languageService.languageIdCodec);
	_onDidChangeLanguage = this._register(new event_1.Emitter());
	onDidChangeLanguage = this._onDidChangeLanguage.event;
	_onDidChangeLanguageConfiguration = this._register(new event_1.Emitter());
	onDidChangeLanguageConfiguration = this._onDidChangeLanguageConfiguration.event;
	_onDidChangeTokens = this._register(new event_1.Emitter());
	onDidChangeTokens = this._onDidChangeTokens.event;
	grammarTokens = this._register(new GrammarTokens(this._languageService.languageIdCodec, this._textModel, () => this._languageId, this._attachedViews));
...
class GrammarTokens extends lifecycle_1.Disposable {
	...
	_backgroundTokenizer = this._register(new lifecycle_1.MutableDisposable());
	_tokens = new contiguousTokensStore_1.ContiguousTokensStore(this._languageIdCodec);
	_debugBackgroundTokens;
	_debugBackgroundStates;
	_debugBackgroundTokenizer = this._register(new lifecycle_1.MutableDisposable());
	_backgroundTokenizationState = 1 /* BackgroundTokenizationState.InProgress */;
	get backgroundTokenizationState() {
		return this._backgroundTokenizationState;
	}
	_onDidChangeBackgroundTokenizationState = this._register(new event_1.Emitter());
	/** @internal, should not be exposed by the text model! */
	onDidChangeBackgroundTokenizationState = this._onDidChangeBackgroundTokenizationState.event;
	_onDidChangeTokens = this._register(new event_1.Emitter());
	/** @internal, should not be exposed by the text model! */
	onDidChangeTokens = this._onDidChangeTokens.event;
	_attachedViewStates = this._register(new lifecycle_1.DisposableMap());
...
class AttachedViewHandler extends lifecycle_1.Disposable {
	_refreshTokens;
	runner = this._register(new async_1.RunOnceScheduler(() => this.update(), 50));
	_computedLineRanges = [];
	_lineRanges = [];
