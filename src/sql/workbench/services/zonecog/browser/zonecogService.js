 ...
 */
const ZONE_COG_SYSTEM_PROMPT = `You are Zone-Cog, an embodied cognition assistant integrated into Azure Data Studio. ` +
 ...
 */
let ZoneCogService = class ZoneCogService extends lifecycle_1.Disposable {
	logService;
	hypergraphStore;
	membraneService;
	llmProviderService;
	_initialized = false;
	_thinkingModeEnabled = true;
	_currentContext = null;
	_cognitiveLoad = 0;
	/** Recent query history for context-aware processing. */
	_queryHistory = [];
	_onDidChangeCognitiveState = this._register(new event_1.Emitter());
	onDidChangeCognitiveState = this._onDidChangeCognitiveState.event;
	_onDidProcessQuery = this._register(new event_1.Emitter());
	onDidProcessQuery = this._onDidProcessQuery.event;
	_onDidCompleteThinkingPhase = this._register(new event_1.Emitter());
	onDidCompleteThinkingPhase = this._onDidCompleteThinkingPhase.event;
	constructor(logService, hypergraphStore, membraneService, llmProviderService) {
		super();
	...
	// -- Query history -------------------------------------------------------
	_recordQueryHistory(query, timestamp) {
	 ...
	 */
	async _runThinkingProtocol(query, complexity, depth) {
		const phases = [];
	...
	_phaseInitialEngagement(query, complexity) {
		const start = Date.now();
		const content = `Hmm, let me clearly understand what's being asked here. ` +
	...
	_phaseProblemSpaceExploration(query, complexity) {
		const start = Date.now();
		const words = query.split(/\s+/);
		const keyTerms = words.filter(w => w.length > 4).slice(0, 5).join(', ');
		const content = `Now let me break this down into its core components. ` +
	...
	_phaseHypothesisGeneration(query) {
		const start = Date.now();
		const content = `Wait, let me think about this from multiple angles before settling on an approach. ` +
	...
	_phaseNaturalDiscovery(query) {
		const start = Date.now();
		const content = `This is interesting because it connects to broader patterns I'm noticing. ` +
	...
	_phaseTestingVerification(query) {
		const start = Date.now();
		const content = `Now let me question my own assumptions here. Am I being too narrow in my analysis? ` +
	...
	_phaseErrorRecognition(query) {
		const start = Date.now();
		const content = `Looking back at my analysis, I want to make sure I haven't made any errors. ` +
	...
	_phaseKnowledgeSynthesis(query) {
		const start = Date.now();
		const content = `Now I'm beginning to see a broader pattern here. Let me connect the different pieces ` +
	...
	_phasePatternRecognition(query) {
		const start = Date.now();
		...
		const content = `Actively looking for patterns in the information I've gathered... ` +
			contextHint +
	 ...
	 */
	_phaseProgressTracking(query, priorPhases) {
		const start = Date.now();
		...
		const content = `Let me take stock of where I am in this analysis. ` +
	 ...
	 */
	_phaseRecursiveThinking(query, priorPhases) {
		const start = Date.now();
		const phaseCount = priorPhases.length;
		const deepestPhase = priorPhases[phaseCount - 1];
		const content = `Now let me apply recursive analysis -- examining my reasoning at multiple scales. ` +
	...
	_phaseResponsePreparation(query, depth) {
		const start = Date.now();
		const content = `Based on this ${depth} analysis, I believe the most helpful response would ` +
	 ...
	 */
	async _generateResponse(query, thinking, complexity) {
	...
	_determineThinkingDepth(complexity) {
		switch (complexity) {
	...
	_calculateConfidence(query, thinking, response, phases) {
		let confidence = 0.65;
		...
		if (this.hypergraphStore.nodeCount() > 5) {
			confidence += 0.05;
		}
		// External LLM provider boosts confidence
		if (this.llmProviderService.isExternalProviderActive()) {
			confidence += 0.05;
		}
...
exports.ZoneCogService = ZoneCogService;
exports.ZoneCogService = ZoneCogService = __decorate([
	__param(0, log_1.ILogService),
	__param(1, zonecogService_1.IHypergraphStore),
	__param(2, zonecogService_1.ICognitiveMembraneService),
	__param(3, llmProvider_1.ILLMProviderService)
], ZoneCogService);
