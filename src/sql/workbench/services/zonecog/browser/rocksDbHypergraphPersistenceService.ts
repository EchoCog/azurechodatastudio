 ...
 * {@link HypergraphPersistenceService}. The pluggable
 * {@link HypergraphPersistenceService.setBackend} `'rocksdb'` path uses
 * {@link RocksDbEngine} directly; this class is the standalone full-service
 * surface on the same engine (distinct from the low-level
 * {@link RocksDbPersistenceService} / {@link IRocksDbPersistenceService} KV API).
 */
export class RocksDbHypergraphPersistenceService extends Disposable implements IHypergraphPersistenceService {
	...
	constructor(
		...
		this._openPromise = this._engine.open()
			.then(() => {
				this._ready = true;
				this.logService.info('RocksDbHypergraphPersistenceService: RocksDB engine opened');
			})
			.catch(err => {
				this._ready = false;
				this.logService.warn(`RocksDbHypergraphPersistenceService: engine open failed - ${err}`);
				throw err;
			});
		...
		return this._serialize(async () => {
			...
			try {
				...
				this.logService.info(
					`RocksDbHypergraphPersistenceService: saved ${nodes.length} nodes, ${links.length} links (label="${label}")`
				);
				...
				this.logService.error(`RocksDbHypergraphPersistenceService: save failed - ${msg}`);
	...
	async load(): Promise<HypergraphSnapshot | undefined> {
		return this._serialize(async () => {
			...
			if (nodes.length === 0 && links.length === 0) {
				this.logService.info('RocksDbHypergraphPersistenceService: nothing stored to load');
				return undefined;
			}
	...
	async clearStorage(): Promise<void> {
		return this._serialize(async () => {
			...
			this.logService.info('RocksDbHypergraphPersistenceService: storage cleared');
		});
	}
	...
	enableAutoSave(intervalMs: number): void {
		...
		this._autoSaveIntervalHandle = setInterval(async () => {
			...
			catch (err) { this.logService.warn(`RocksDbHypergraphPersistenceService: auto-save failed - ${err}`); }
		}, ms);
	}
	...
	async setBackend(kind: HypergraphPersistenceBackendKind): Promise<void> {
		if (kind !== 'rocksdb') {
			throw new Error(`RocksDbHypergraphPersistenceService only supports the 'rocksdb' backend (requested '${kind}')`);
		}
	}
	...
	async compactStorage(): Promise<void> {
		...
		this.membraneService.recordActivity('autonomic');
		this.logService.info('RocksDbHypergraphPersistenceService: compaction complete');
	}
	...
	private _recordChange(entityType: 'node' | 'link', entityId: string): void {
		...
		this._changeLogWriteQueue = this._changeLogWriteQueue
			...
			.catch(err => this.logService.warn(`RocksDbHypergraphPersistenceService: failed to record changelog entry - ${err}`));
	}
