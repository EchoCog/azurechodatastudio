...
exports.PerformanceAdvisorAgent = void 0;
 ...
 */
let PerformanceAdvisorAgent = class PerformanceAdvisorAgent extends lifecycle_1.Disposable {
	logService;
	membraneService;
	hypergraphStore;
	_serviceBrand;
	id = 'performance-advisor-agent';
		...
		try {
			switch (action.action) {
				case 'analyze_performance':
					return await this.analyzePerformance(action.target, typeof action.parameters?.executionPlan === 'string' ? action.parameters.executionPlan : undefined);
	...
	async analyzePerformance(query, executionPlan) {
		this.membraneService.recordActivity('cerebral');
	...
	async identifyAntiPatterns(query) {
		this.membraneService.recordActivity('cerebral');
	...
	_extractColumnUsage(query, usage) {
	...
	_estimateImprovement(usageCount, totalQueries) {
	...
	async _storeAnalysis(query, issues) {
		const node = {
...
exports.PerformanceAdvisorAgent = PerformanceAdvisorAgent;
exports.PerformanceAdvisorAgent = PerformanceAdvisorAgent = __decorate([
