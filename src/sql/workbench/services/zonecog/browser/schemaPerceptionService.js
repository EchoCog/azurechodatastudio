...
exports.SchemaPerceptionService = void 0;
 ...
 */
let SchemaPerceptionService = class SchemaPerceptionService extends lifecycle_1.Disposable {
	logService;
	hypergraphStore;
	membraneService;
	embodiedService;
	...
	_activePerceptions = new Set();
	// Element access counts for salience calculation
	_accessCounts = new Map();
	_queryCounter = 0;
	_onDidPerceiveSchema = this._register(new event_1.Emitter());
	onDidPerceiveSchema = this._onDidPerceiveSchema.event;
	_onDidObserveQuery = this._register(new event_1.Emitter());
	onDidObserveQuery = this._onDidObserveQuery.event;
	constructor(logService, hypergraphStore, membraneService, embodiedService) {
		super();
	...
	// -- Schema Perception ----------------------------------------------------
	async perceiveSchema(connectionUri) {
		...
		try {
			...
			this.embodiedService.perceive('schema', `Schema perception completed for ${connectionUri}`, JSON.stringify({ connectionUri, elementCount: elements.length }), 0.6);
			return elements;
		}
		catch (error) {
	...
	getPerceivedElements(connectionUri) {
	...
	getElementsByType(connectionUri, elementType) {
		const elements = this.getPerceivedElements(connectionUri);
		return elements.filter(e => e.elementType === elementType);
	}
	searchElements(connectionUri, pattern) {
		const elements = this.getPerceivedElements(connectionUri);
		const lowerPattern = pattern.toLowerCase();
		return elements.filter(e => e.name.toLowerCase().includes(lowerPattern) ||
			e.qualifiedName.toLowerCase().includes(lowerPattern));
	}
	// -- Query Observation ----------------------------------------------------
	observeQuery(queryData) {
		this.membraneService.recordActivity('somatic');
		const query = {
		...
		this.embodiedService.perceive('query', `${query.operationType.toUpperCase()}: ${query.queryText.substring(0, 100)}`, JSON.stringify({
		...
		this.logService.trace(`SchemaPerceptionService: observed ${query.operationType} query (${query.durationMs}ms, ${query.rowCount} rows)`);
		return query;
	}
	getRecentQueries(connectionUri, limit = 50) {
		const queries = this._queries.get(connectionUri) ?? [];
		return queries.slice(-limit);
	}
	getQueriesForTable(tableName) {
		const result = [];
	...
	getQueryStatistics(connectionUri) {
		const queries = this._queries.get(connectionUri) ?? [];
		const stats = {
	...
	async registerSchemaInHypergraph(connectionUri) {
		...
		this.logService.info(`SchemaPerceptionService: registered ${nodesCreated} schema nodes in hypergraph for ${connectionUri}`);
		return nodesCreated;
	}
	getElementSalience(elementId) {
	 ...
	 */
	parseQueryOperationType(queryText) {
	 ...
	 */
	extractReferencedTables(queryText) {
		const tables = [];
...
exports.SchemaPerceptionService = SchemaPerceptionService;
exports.SchemaPerceptionService = SchemaPerceptionService = __decorate([
