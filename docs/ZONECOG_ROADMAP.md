...
- [x] Incremental backup/restore (remainder of `E.3`) — `createBackup()`/`exportBackupJson()` produce a portable JSON `HypergraphBackup`, full or (given a `sinceTimestamp`) an incremental delta of just the nodes/links upserted since then, computed from an append-only IndexedDB changelog fed by `IHypergraphStore.onDidChangeNode`/`onDidChangeLink`; `importBackup()`/`importBackupJson()` apply one by upserting into the live hypergraph and hot tier, so a full backup plus a chain of incrementals reconstructs state. Exposed via `Zone-Cog: Export Hypergraph Backup` / `Zone-Cog: Import Hypergraph Backup` Command Palette actions. Deltas only cover upserts (removal outside this service isn't observable via those events, so periodic full backups remain the source of truth for deletions); cloud storage integration remains future work (needs a chosen provider and credential handling, out of scope for an unattended pass)
